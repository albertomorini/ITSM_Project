...
Another Key Performance Indicator which can help the IT Department toward the monitor the performance of the service within the expectation of the stakeholders is the measurement of the restoring service time.
This indicator measures how quickly the IT organization restores service after an interruption, closely related to the previous metric~\ref{sub:slaComplianceRate} but can be adopted by IT Department as an input for continuous improvement toward to optimize it's response for incidents.
...
% The current number of open incidents, problems, change requests or others kinds that are awaiting for completation.
% This indicators can provide a clear and immediate panoramic of the current status of each department and situation.
...
Backlog priority distribution measure the compositiion of open ticket, grouping them by their priority. This metric can be applied to different subject, such as problems or change requests.

With this KPI, the IT can monitor a rank of the amount of task for each division thus to plan and coordinate different resources in order to accomplish first critical tasks and to direct others requests.

It affects several ITSM process for each division, for instance:
...
Combined with the metric before \ref{sub:prioritydistribution}, provide the work tasks successfully completed during the reporting period.
